...
//    }
    ...
    bool print =  false;//event.id().event() == 36924813;
    if (event.id().event()== 112) std::cout << "                            ---------------- event, run, lumi = " << event.id().event() << " " << event.id().run() << " " << event.id().luminosityBlock() << "----------------" << std::endl;
    int nPions = 0;
    for( size_t ii = 0; ii < packedpfcandidates_->size(); ++ii ){   
      ...
      if (print) std::cout << "---------------- event, run, lumi, pf pT = " << event.id().event() << " " << event.id().run() << " " << event.id().luminosityBlock() << " " << pf.pt() << "----------------" << std::endl;

      if(TMath::Abs(pf.pdgId())!=211) {if (print) cout << "Not a charged pion" << endl; continue;}
      nPions += 1;
