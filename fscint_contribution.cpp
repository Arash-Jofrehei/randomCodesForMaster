...
double fit_function(double *v,double *par)
{
  ...
  //return par[0]*convoluted_pulse_fscint->GetBinContent(n+1+shift)+par[1]*convoluted_pulse_WLS->GetBinContent(n+1+shift)+par[3];
}
void fscint_contribution(){
  ...
  if (save){
    for (Long64_t jentry=0; jentry<nentries;jentry++){
      ...
      for (int sample = 0;sample < 1024;sample++){
        apd[0][x_index][y_index]->Fill(30+WF_time6[sample]-Time[6],WF_val6[sample]);
        apd[1][x_index][y_index]->Fill(30+WF_time7[sample]-Time[7],WF_val7[sample]);
        apd[2][x_index][y_index]->Fill(30+WF_time8[sample]-Time[8],WF_val8[sample]);
        apd[3][x_index][y_index]->Fill(30+WF_time9[sample]-Time[9],WF_val9[sample]);
      }
      for (int sample = 866;sample < 1024;sample++){
        for (int a = 0;a < 4;a++){
          //apd[a][x_index][y_index]->Fill(sample/5.0,0);
  ...
  for (int i = 0;i < nbins;i++){
    for (int j = 0;j < nbins;j++){
      ...
      for (int a = 0;a < 4;a++){
        ...
        //apd[a][i][j]->Write();
        func->SetParameters(1,0.5,70,0,0);
        for (int k = 0;k < 10;k++){
          apd[a][i][j]->Fit("fit","","",30,80);
        }
  ...
  for (int i = 0;i < nbins;i++){
    for (int j = 0;j < nbins;j++){
      cout << endl << i+1 << "    " << j+1 << "      " << ratio_hist2D_apd[0]->GetBinContent(i+1,j+1);
    }
  }
  cout << endl;
  func->SetParameters(1,1,70,0,0);
  for (int i = 0;i < 10; i++){
    apd[0][18][18]->Fit("fit","","",30,80);
  }
