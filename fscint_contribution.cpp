...
void fscint_contribution(){
  ...
  double c_fscint = 1;
  double c_WLS = 1;
  TH2F *apd1_ratio_hist2D = new TH2F("apd1_ratio_hist2D","apd1_ratio_hist2D",nbins,197,217,nbins,284.5,304.5);
  /*TH2F ***apd1;
  string name;
  ...
  for (int i = 0;i < nbins;i++){
    for (int j = 0;j < nbins;j++){
      ...
      for (Long64_t jentry=0; jentry<nentries/10;jentry++){
        ...
        if (x[1]<197||x[1]>=217||y[1]<284.5||y[1]>=304.5) continue;
        int x_index = int((nbins/20.0)*(x[1]-197));
        int y_index = int((nbins/20.0)*(y[1]-284.5));
